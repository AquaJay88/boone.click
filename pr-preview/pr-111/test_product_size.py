...
def run(playwright):
    ...
    # Intercept Supabase calls to return mock data
    def handle_route(route):
        ...
        if "supabase" in url:
            if "select" in url and "product_media" in url:
                ...
                route.fulfill(status=200, content_type="application/json", body='[{"product_id": "1", "name": "Test Item", "price_display": "10.00", "stripe_price_id": "price_1", "size_label": "One Size", "variation_type": "Color"}]')
            else:
                route.continue_()
        else:
            route.continue_()
    ...
    # Wait for the product title to load so we know the JS has executed
    page.wait_for_selector("#productTitle:not(:has-text('Loading...'))")
    page.wait_for_timeout(2000)

    # Check if sizeGroup is hidden
    ...
    print(f"Size Group Hidden: {is_hidden}")

    # Check if state has the right values
    size_val = page.evaluate("state.sizeVal")
    price_id = page.evaluate("state.priceId")
    display_price = page.evaluate("state.displayPrice")
    print(f"State sizeVal: {size_val}")
    print(f"State priceId: {price_id}")
    print(f"State displayPrice: {display_price}")
